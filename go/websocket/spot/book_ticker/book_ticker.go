// Package bookticker implements Binance Spot book-ticker subscriptions.
package bookticker
...
func (p Params) Stream() (string, error) {
	...
	if symbol == "" {
		return "", fmt.Errorf("failed to build Spot book ticker stream: symbol=empty")
	}
...
func NewClient(executor subscriptions.Executor) (*Client, error) {
	if executor == nil {
		return nil, fmt.Errorf("failed to create Spot book ticker subscription client: executor=null")
	}
...
func (c *Client) send(ctx context.Context, method string, params Params) error {
	...
	if c == nil || c.executor == nil {
		return fmt.Errorf("failed to %s Spot book ticker: client=null", operation)
	}
	stream, err := params.Stream()
	if err != nil {
		return fmt.Errorf("failed to %s Spot book ticker: %w", operation, err)
	}
	...
	if err != nil {
		return fmt.Errorf("failed to %s Spot book ticker: failed to encode request: %w", operation, err)
	}
	if method == wsprotocol.MethodSubscribe {
		if err := c.executor.Subscribe(ctx, stream, payload); err != nil {
			return fmt.Errorf("failed to subscribe Spot book ticker: %w", err)
		}
		return nil
	}
	if err := c.executor.Unsubscribe(ctx, stream, payload); err != nil {
		return fmt.Errorf("failed to unsubscribe Spot book ticker: %w", err)
	}
	return nil
}
